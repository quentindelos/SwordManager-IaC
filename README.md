...
* **Réseau (VPC) :** Un réseau VPC dédié (`swordmanager-vpc`) isole les composants de l'application. Une règle de pare-feu (`allow-postgres`) qui autorise uniquement le trafic entrant sur le port `5432` pour la base de données.
* **Frontend (`swordmanager-frontend`) :** Hébergé sur **Google Cloud Run**, conteneurisé avec Nginx pour l'application. Il est associé aux domaines `swordmanager.cloud` et `www.swordmanager.cloud`.
...
└── SwordManager-IaC/
    ...
    │   │
    │   └── qualif/
    │   │   ├── main.tf                 # Point d'entrée, appel des modules pour l'env Qualif
    │   │   ├── variables.tf            # Déclaration des variables propres à l'env Qualif
    │   │   ├── outputs.tf
    │   │   └── terraform.tfvars
    │   │
    │   └── prod/
    │       ├── main.tf                 # Point d'entrée, appel des modules pour l'env Prod
    │       ├── variables.tf            # Déclaration des variables propres à l'env Prod
    │       ├── outputs.tf
    │       └── terraform.tfvars
    │
    └── modules/
        ...
        │
        └── analytics/
            ├── main.tf                 # BigQuery pour la centralisation des logs
            └── variables.tf
```